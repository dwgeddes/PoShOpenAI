# Pester test suite for PSOpenAI module

Describe 'PSOpenAI Module Import and Export' {
    It 'Should import the module without error' {
        { Import-Module "$PSScriptRoot/../PSOpenAI.psd1" -Force -ErrorAction Stop } | Should -Not -Throw
    }
    It 'Should export all expected public functions' {
        Import-Module "$PSScriptRoot/../PSOpenAI.psd1" -Force
        $expected = @(
            'Invoke-OpenAIPrompt',
            'Send-ChatMessage',
            'New-OpenAIImage',
        ...
        $exported = (Get-Command -Module PSOpenAI).Name
...
Describe 'Invoke-OpenAIPrompt Parameter Validation' {
    BeforeAll { Import-Module "$PSScriptRoot/../PSOpenAI.psd1" -Force }
